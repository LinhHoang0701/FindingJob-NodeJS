...
exports.connections = async (req, res, next) => {
  ...
  try {
    ...
    if (!userAccount)
      throw new APIError(
        `No user associated with id: ${connectToUser}`,
        httpStatus.NOT_FOUND
      );
    ...
    let result = await neo4jSession.run(
      "MATCH (a {userid:{emailadd}})--(b) return b",
      { emailadd: uId }
    );
    ...
    for (let index = 0; index < result.records.length; index++) {
      ...
      let user = await Applicant.findOne({
        id: JSON.parse(temp)._fields[0].properties.userid,
      }).exec();
      if (!user) {
...
exports.connect = async (req, res, next) => {
  ...
  try {
    ...
    let already = await tx.run(
      "Match (a {userid:{loggedIn}})-[:FRIEND]-(b {userid:{connectTo}}) return a,b",
      { loggedIn: loggedInUser, connectTo: connectToUser }
    );
    if (already.records.length !== 0) {
      throw new APIError("Already Connected to this user");
    }
    let r1 = await tx.run("MATCH (a {userid:{loggedIn}}) return a.userid", {
      loggedIn: loggedInUser,
    });
    console.log("1std", r1);
    if (r1.records.length !== 0) {
      let r2 = await tx.run("MATCH (b {userid:{connectTo}}) return b.userid", {
        connectTo: connectToUser,
      });
      console.log("2nd", r1);
      if (r2.records.length !== 0) {
        await tx.run(
          "Match (a {userid:{loggedIn}}),(b {userid:{connectTo}}) MERGE (a)-[:FRIEND]-(b) MERGE (b)-[:FRIEND]-(a) return a,b",
          { loggedIn: loggedInUser, connectTo: connectToUser }
        );
      } else {
        await tx.run(
          "CREATE (n:Person {userid:{email},type:{UserType}}) RETURN n.userid",
          { email: connectToUser, UserType: connectToUserType }
        );
        await tx.run(
          "Match (a {userid:{loggedIn}}),(b {userid:{connectTo}}) MERGE (a)-[:FRIEND]-(b) MERGE (b)-[:FRIEND]-(a) return a,b",
          { loggedIn: loggedInUser, connectTo: connectToUser }
      ...
      await tx.run(
        "CREATE (n:Person {userid:{email},type:{UserType}}) RETURN n.userid",
        { email: loggedInUser, UserType: loggedInUserType }
      );

      let r3 = await tx.run("MATCH (b {userid:{connectTo}}) return b.userid", {
        connectTo: connectToUser,
      });
...
exports.mutual = async (req, res, next) => {
  ...
  try {
    ...
    var obj = {};
    let result = await neo4jSession.run(
      "MATCH (a {userid:{emailadd}})--(b)--(c) return c",
      { emailadd: uId }
    );
    console.log("rr", result);
    for (let index = 0; index < result.records.length; index++) {
...
exports.getProfile = async (req, res, next) => {
  try {
    ...
    const userTranform = userDoc.transform();
    let userDocs;
    if (userTranform.role == "applicant") {
      userDocs = await Applicant.find({ id: userTranform.id });
    } else if (userTranform.role == "recruiter") {
      userDocs = await Recruiter.find({ id: userTranform.id });
    } else {
      userDocs = await Admin.find({ id: userTranform.id });
    }
    res.status(200).json({
      user: userDocs[0],
    });
